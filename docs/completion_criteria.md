...
| Production threading | Expensive visual exterior search, sand accumulation, mesh-change rebuild, and texture upload work must not block the GameThread at production character scale. |
...
- Production Unreal work still needs a worker/render-thread implementation, dirty 3D texture upload path, and off-GameThread exterior proxy rebuild from immutable mesh snapshots.

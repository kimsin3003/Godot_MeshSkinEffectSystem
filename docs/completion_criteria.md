...
| Animation/skinning | Either the sampler must account for skinned deformation, or the documented approximation must be proven acceptable on animated real characters. |
...
- Animated/skinned approximation is covered by `tests/render_animated_real_asset_smoke.gd`: Sophia's Run animation changes skeleton pose and still renders visible impact pixels.

...
latest benchmark:  hits=8 raycast_ms=7.629  event_ms=0.222  full_ms=7.852
```
...
- 최적화 후에도 click당 약 8.0 ms가 걸리며, 대부분은 exact visual raycast다. 이 수치는 실제 게임 hit path로 받아들이면 안 된다.
- Unreal 게임에서는 physics/collision hit를 입력으로 받고, visual exterior 보정만 별도 proxy에서 수행해야 한다.

누적 scaling 측정:

```text
impact batches: hits_avg_ms=0.231,0.148,0.144,0.140 for 1/32/128/512 hit batches
sand before:    sand_step_ms=13.699,43.616,93.483,217.821,227.856,263.155,259.385,259.295
sand latest:    sand_step_ms=4.982,1.927,4.385,5.288,4.292,0.810,0.785,0.568
sand frames:    sand_frame_avg_ms=2.341 sand_frame_max_ms=5.147
rebuild:        repeated rebuild_ms=8.928,9.391,12.150,15.651 swap_ms=196.158
```
...
| Exterior surface proxy | Physics hit를 visual outer layer로 보정 | event-driven mesh rebuild/clothing swap 때 재생성 |
...
Unreal에서는 매 frame 40만 vertex나 모든 proxy sample을 GameThread에서 훑으면 안 된다. 또한 front plane 뒤쪽 전체가 아니라 wind-exposed outer shell만 누적해야 한다. 권장 방향:
...
## Mesh Change and Threading Boundary

Mesh 변경 감지는 매 frame 전체 mesh tree나 vertex buffer를 훑어서 찾지 않는다. Character assembly/clothing swap/runtime mesh generation이 끝나는 지점에서 surface-effect component에 명시적으로 rebuild를 요청하고 generation id를 증가시킨다.

권장 흐름:

| Thread | Responsibility |
| --- | --- |
| GameThread | mesh generation 완료 이벤트 수신, component transform/immutable mesh snapshot 참조 수집, generation id 증가, job enqueue, 완료 결과 적용 |
| WorkerThread | exterior proxy build/search, rest-space bounds 계산, CPU volume buffer 초기화/splat, dirty region 계산 |
...
- GameThread에서 허용되는 rebuild work는 snapshot pointer 수집, 작은 parameter update, job enqueue 정도로 제한한다.

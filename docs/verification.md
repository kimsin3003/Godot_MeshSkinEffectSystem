...
| Playtest hit performance baseline | `tests/benchmark_playtest_hit.gd` measures the current Godot playtest path. Latest run: `hits=8`, `raycast_ms=7.792`, `event_ms=0.228`, `full_ms=8.021`. The remaining cost is the prototype/editor-style exact visual raycast path, not the intended Unreal GameThread path. |
| Accumulation scaling | `tests/benchmark_accumulation_scaling.gd` proves impact cost stays stable as events accumulate (`hits_avg_ms=0.230,0.147,0.143,0.146` for 1/32/128/512 hit batches). Sand now uses an incremental front cursor, wind-exposed outer-shell filtering, and capped accumulation samples: latest `sand_frame_avg_ms=2.367`, `sand_frame_max_ms=8.795` on the Quaternius playtest asset. |

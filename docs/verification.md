...
| Artist material event contract | `tests/test_surface_effects.gd` calls `add_surface_effect(7, ...)` and asserts every material slot receives `impact_spheres`, `impact_dirs`, and `impact_meta.x == 7`; it also verifies the event is accumulated into the fourth O(1) volume channel. |
| In-game effect playtest | `scenes/effect_playtest.tscn` loads the real Quaternius character, raycasts left-clicks against current skinned visual mesh triangles, and calls `add_surface_effect_at_triangle()` so the material event is attached to the clicked triangle/barycentric coordinate. Right-click starts sand wind from the camera direction toward the clicked surface, while `T`/`F`/`Q`/`E` control sand playback and direction. `tests/test_effect_playtest_scene.gd` verifies the scene builds `13456` samples, stays at `766336` bytes, a center-screen click adds one event with `click_error=0.0000`, and `start_sandstorm(Vector3(1,0,0))` pushes the sand direction to materials. |
...
| Material O(1) impact evaluation | `surface_effects.gdshader` uses `surface_effect_volume` as the default impact path, so fragment cost is one 3D texture sample instead of `impact_count` event iterations. The old event-array loop remains behind `use_surface_effect_volume == false` for compatibility/debug materials. |
| Animated/skinned deformation attachment | `tests/test_skinned_surface_attachment.gd` attaches an event to a real Sophia triangle using surface index, triangle indices, and barycentric coordinates, then seeks the `Run` animation and proves the event center follows the skinned deformation (`movement=0.592`, `attachment_delta=0.592`). `tests/render_animated_real_asset_smoke.gd` still renders the animated real-character case and checks visible impact metrics against `tests/visual_baselines.json`. |
| Runtime vertex deformation attachment | `tests/test_deformed_surface_attachment.gd` supplies a `deformation_provider` that changes one triangle vertex after the event is recorded, then proves the material event center follows that deformed vertex data (`delta=0.200`). This is the extension path for cloth, procedural deformation, or GPU/readback-fed vertices. |
...
- Attached triangle events are updated from current skinned triangle positions, or from `deformation_provider.resolve_surface_vertex_world()` when a runtime deformation source is supplied. Physics-only events that do not carry triangle/barycentric data still use the cheaper character-local fallback.
- The O(1) volume path currently exposes four accumulated effect classes through RGBA. More independently styled effect classes would need more volume channels/textures or a different packed representation.

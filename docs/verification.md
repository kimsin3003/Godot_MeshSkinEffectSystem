...
| Accumulation scaling | `tests/benchmark_accumulation_scaling.gd` proves impact cost stays stable as events accumulate (`hits_avg_ms=0.231,0.148,0.144,0.140` for 1/32/128/512 hit batches). Sand now uses an incremental front cursor, wind-exposed outer-shell filtering, and capped accumulation samples: latest `sand_frame_avg_ms=2.341`, `sand_frame_max_ms=5.147` on the Quaternius playtest asset. |

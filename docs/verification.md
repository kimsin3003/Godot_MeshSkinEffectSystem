...
| Material O(1) impact evaluation | `surface_effects.gdshader` uses `surface_effect_volume` as the default impact path, so fragment cost is one 3D texture sample instead of an event loop. The old array loop remains only behind `use_surface_effect_volume == false`. |

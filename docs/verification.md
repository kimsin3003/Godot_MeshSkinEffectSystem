...
| Accumulation scaling | `tests/benchmark_accumulation_scaling.gd` proves impact cost stays stable as events accumulate (`hits_avg_ms=0.213,0.140,0.145,0.144` for 1/32/128/512 hit batches). Sand now uses an incremental front cursor, wind-exposed outer-shell filtering, capped accumulation samples, and a 1-voxel cross brush to reduce sparse-sample striping: latest `sand_frame_avg_ms=4.455`, `sand_frame_max_ms=11.083` on the Quaternius playtest asset. |

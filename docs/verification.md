...
| --- | --- |
| Runtime character mesh can be rebuilt cheaply | `SurfaceEffectAccumulator.rebuild_for_character()` clears accumulated volume/event state, rebuilds sparse samples, injects rest `CUSTOM0`, and rebinds per-surface shader materials without asset writes. It skips old-volume upload during rebuild, avoids reinjecting already-rest meshes, and samples mesh arrays in one pass. Latest `tests/benchmark_rebuild_cost.gd`: repeated `rebuild_ms=15.682,16.027,16.210,15.256`, swap path `swap_ms=179.581`. |
...
| Current validation assets stay below 1 MB | Synthetic and real tests assert `estimate_memory_bytes() < 1024 * 1024`. Latest real values: Sophia `776688`, Kenney `484552`, Quaternius layered validation `981632`, Quaternius playtest `820160`, Soldier playtest after swap `879000`. |
...
| In-game effect playtest | `scenes/effect_playtest.tscn` loads the real Quaternius character. Left-click raycasts current visual mesh triangles and calls `add_surface_effect_at_triangle()`. Right-click starts sand wind from camera direction. Latest passing `tests/test_effect_playtest_scene.gd`: `samples=6728`, `memory=820160`, `events=1`, `click_error=0.0000`. |
...
| Sand comes from a direction and respects normal angle | `tests/render_sand_mask_smoke.gd` renders the fallback shader path and samples pixels. Latest values: `left_early=0.084`, `left_late=0.669`, `right_late=0.084`, `parallel_late=0.084`. It also verifies procedural sand does not leak into the accumulated volume path (`volume_probe=0.084`). |
...
| Playtest hit performance baseline | `tests/benchmark_playtest_hit.gd` measures the current Godot playtest path. Latest run: `hits=8`, `raycast_ms=7.629`, `event_ms=0.222`, `full_ms=7.852`. The remaining cost is the prototype/editor-style exact visual raycast path, not the intended Unreal GameThread path. |
| Accumulation scaling | `tests/benchmark_accumulation_scaling.gd` proves impact cost stays stable as events accumulate (`hits_avg_ms=0.219,0.138,0.142,0.142` for 1/32/128/512 hit batches). Sand now uses an incremental front cursor, wind-exposed outer-shell filtering, capped accumulation samples, deterministic jittered surface samples, a 1-voxel sand brush, and fixed 13-tap tangent-gated material smoothing for the sand channel: latest `sand_frame_avg_ms=4.656`, `sand_frame_max_ms=15.153` on the Quaternius playtest asset. |
...
- Godot rebuild is still a synchronous prototype path. It avoids repeated rest mesh injection and redundant volume uploads, but production Unreal should build exterior proxies and volume state from immutable mesh snapshots off the GameThread.

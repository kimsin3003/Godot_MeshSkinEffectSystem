...
3. 정확한 visual hit 정보를 사용할 수 있으면 surface index, triangle index, barycentric 좌표를 함께 저장해 애니메이션/skinning 후에도 현재 변형된 triangle 위치에서 event center를 재구성한다.
   cloth, procedural, GPU readback 같은 별도 변형 소스가 있으면 `deformation_provider.resolve_surface_vertex_world()`로 현재 vertex world 위치를 공급한다.
4. 찾은 로컬 위치, 반경, 방향, 강도, effect id를 작은 ring buffer에 저장한다.
5. 모든 material slot에 같은 surface event uniform 배열을 전달하고, shader가 로컬/월드 위치와 normal로 피격 splat과 모래 누적량을 계산한다.
...
- O(1) material 평가용 48³ RGBA8 volume: 약 432 KB
...
기본 material은 shader fragment에서 O(1)로 동작하도록 다음 volume uniform을 먼저 사용한다.

- `use_surface_effect_volume`: true이면 O(1) volume path 사용
- `surface_effect_volume`: 48³ RGBA8 3D texture. R/G/B/A는 effect class 1/2/3/4의 누적 mask
...
호환성/디버그/커스텀 material용으로 raw event uniform도 유지한다.
...
기본 shader는 volume을 한 번 샘플해서 effect class별 색상/roughness 변화를 적용한다. 아티스트는 O(1) path에서는 RGBA mask를, 더 많은 raw event 정보가 필요한 커스텀 material에서는 event uniform 배열을 사용할 수 있다.
...
- 더 많은 event 수를 위한 data texture / structured buffer 기반 event storage
